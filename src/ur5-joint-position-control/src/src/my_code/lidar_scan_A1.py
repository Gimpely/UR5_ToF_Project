...
def callback(A1):
    ...
    # Publish array_min_min on the A1_sim topic
    msg = Int32Stamped()
    msg.header = Header()
    msg.header.stamp = rospy.Time.now()
    msg.data = array_min_min
    pub_A1_sim.publish(msg)
...
ts = message_filters.ApproximateTimeSynchronizer([A1], 1, 1)
ts.registerCallback(callback)
# Create a new publisher for the A1_sim topic
pub_A1_sim = rospy.Publisher('A1_sim', Int32Stamped, queue_size=10)
...
# #Write to csv - change name
# def write_2_csv(data):
#     with open(rospack.get_path('myur5_description')+'/scripts/lidar_data/lidar_data_A1.csv', 'a') as file:
...
# print('Program starts after 2s.')
# time.sleep(2)
# test_data = (message_filters.Subscriber('laser_frame_test', LaserScan))
# ts = message_filters.ApproximateTimeSynchronizer([test_data], 1, 1)
# ts.registerCallback(callback)
# print('Writing data to csv file.')
